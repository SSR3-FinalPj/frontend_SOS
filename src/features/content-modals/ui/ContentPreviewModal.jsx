...
import { get_traffic_source_summary, getCommentAnalysis } from '@/common/api/api';
...
const ContentPreviewModal = ({ 
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!item || viewMode !== 'detailed') return;
      setLoading(true);
      try {
        const [trafficData, commentData] = await Promise.all([
          get_traffic_source_summary(item.videoId),
          getCommentAnalysis(item.videoId)
        ]);
